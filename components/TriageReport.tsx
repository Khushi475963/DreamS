...
const TriageReport: React.FC<Props> = ({ data, intakeData }) => {
  ...
  const handleDownloadPdf = async () => {
    ...
    const opt = {
      ...
      filename: `MiraiHealth_Report_${intakeData?.fullName || 'Patient'}_${new Date().toISOString().slice(0,10)}.pdf`,
      ...
      <div ref={reportRef} id="triage-report-content" className="space-y-6 bg-slate-50 p-2 md:p-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6 break-inside-avoid">
            ...
            <div className="bg-gradient-to-br from-teal-600 to-teal-700 rounded-xl p-5 shadow-lg text-white">
              ...
              <p className="text-teal-100 text-sm mt-1 opacity-90">Based on symptom presentation</p>
            </div>
        ...
        <div className="hidden print:block text-center text-xs text-slate-400 mt-8 pt-4 border-t border-slate-200">
          Generated by Mirai Health² Clinical Triage • {new Date().toLocaleDateString()}
        </div>
      </div>
    </div>
  );
